<?php
	if(isset($_POST['save']))
		{ // jika tombol 'Simpan' ditekan
			...
			$cek_sk_wisuda = mysqli_query($koneksi, "SELECT * FROM sk_wisuda WHERE (nim = '$nim') ");

			
			if(mysqli_num_rows($cek_sk_wisuda) != 0)
			{
				
				$data=mysqli_fetch_array($cek_sk_wisuda);
							...
							{ 
								...
								if(mysqli_num_rows($cek) == 0)
									{ // mengecek apakah no pendaftaran tidak ada dalam database
										$insert = mysqli_query($koneksi, "INSERT INTO pendaftar_wisuda_rega (program, ta_sem, prodi, nim, nama, tmpt_lhr, tgl_lhr, alamat, jenkel, ipk, judul_indo, judul_english, pesan, foto, no_sk) VALUES ('$program', '$ta_sem', '$prodi', '$nim', '$nama', '$tmpt_lhr', '$tgl_lhr', '$alamat', '$jenkel', '$ipk', '$judul_indo', '$judul_english', '$pesan', '$fotobaru', '$no_sk' ) ") or die(mysqli_error()); // query untuk menambahkan data ke dalam database
			...
			{
				echo"<script>alert('Anda belum bisa Mendaftar Wisuda, karena belum terdaftar dalam SK')</script>";
