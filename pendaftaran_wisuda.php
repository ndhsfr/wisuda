        	...
        	<form class="form-horizontal" action="" method="post" enctype="multipart/form-data">								
				<div class="panel panel-default">
	  				...
	  				<div class="panel-body">
						...
						<?php
						   include("pendaftar_reg-A/proses_save-3.php"); // memanggil file proses simpan
						?>
						...
						<!-- bagian ini merupakan bagian form untuk menginput data yang akan dimasukkan ke database -->
							...
							<div class="form-group">
								...
								<div class="col-sm-7">
									<textarea name="t_pesan" id="t_pesan" class="form-control" placeholder="Kesan Dan pesan Selama menjadi Mhs di unimugo" required></textarea>
								</div>
							</div>	
							<div class="form-group">
	...
	$(document).ready(function() {
		...
		$('#btn-save').click(function() { // Jika tombol button simpan - di klik
		 ...
		 var 
			...
			parameter = "TA=" + $(s_TA).val() + "&nim=" + $(t_nim).val() + "&nama=" + $(t_nama).val() 
			 + "&tmpt_lhr=" + $(t_tmpt_lhr).val() + "&tgl_lhr=" + $(t_tgl_lhr).val() 
			 + "&alamat=" + $(t_alamat).val() + "&jenkel=" + $(t_jenkel).val() 
			 + "&ipk=" + $(t_ipk).val() + "&judul_indo=" + $(t_judul_indo).val()
			 + "&judul_english=" + $(t_judul_english).val() + "&pesan=" + $(t_pesan).val() ; 
		 
		if ( ($(s_TA).val() != "") & ($(t_nim).val() != "") & ($(t_nama).val() != "") & ($(t_tmpt_lhr).val() !="") & ($(t_tgl_lhr).val() != "") & ($(t_alamat).val() != "") & ($(t_jenkel).val() != "") & ($(t_ipk).val() != "") & ($(t_judul_indo).val() != "") & ($(t_judul_english).val() != "") & ($(t_pesan).val() != "") )
			{
		 	 $.ajax({
					type: 'POST', // Metode pengiriman data menggunakan POST
					url: 'pendaftaran_reg-A/proses_save_pendaftar_reg-A_ajax.php', // File yang akan memproses data
					//data:  data,
					data: parameter,
					//processData: false,
					//contentType: false,
					...
					{ 								
						if(response.status == "sukses") // Jika Statusnya = sukses
							...
							{ 
								alert ("sudah ada no pendaftaran");							
							}
