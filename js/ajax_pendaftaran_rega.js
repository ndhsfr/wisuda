$(document).ready(function(){
	...
	$("#btn-simpan").click(function(){ // Ketika tombol simpan di klik
		...
		data.append('TA_sem', $("#TA_sem").val()); // Ambil data TA_sem
		data.append('prodi', $("#prodi").val()); // Ambil data prodi
		data.append('nim', $("#nim").val()); // Ambil data nim
		data.append('nama', $("#nama").val()); // Ambil data nama
		data.append('tmpt_lhr', $("#tmpt_lhr").val()); // Ambil data tempat lahir
		data.append('tgl_lhr', $("#tgl_lhr").val()); // Ambil data tgl lhr
		data.append('alamat', $("#alamat").val()); // Ambil data alamat
		data.append('jenkel', $("#jenkel").val()); // Ambil data jenis kelamin
		data.append('ipk', $("#ipk").val()); // Ambil data ipk
		data.append('judul_indo', $("#judul_indo").val()); // Ambil data judul indo
		data.append('judul_english', $("#judul_english").val()); // Ambil data judul english
		data.append('pesan', $("#pesan").val()); // Ambil data pesan
		...
		$.ajax({
			url: 'pendaftar_reg-A/proses_simpan.php', // File tujuan
	...
	$("#btn-ubah").click(function(){ // Ketika tombol ubah di klik
		...
		$.ajax({
			url: 'pendaftar_reg-A/proses_ubah.php', // File tujuan
			type: 'POST', // Tentukan type nya POST atau GET
			data: data, // Set data yang akan dikirim
			processData: false,
			contentType: false,
	...
	$("#btn-hapus").click(function(){ // Ketika tombol hapus di klik
		...
		$.ajax({
			url: 'pendaftar_reg-A/proses_hapus.php', // File tujuan
			type: 'POST', // Tentukan type nya POST atau GET
			data: data, // Set data yang akan dikirim
			processData: false,
			contentType: false,
